...
// 这两个设置 Modal 使用复合组件模式，需要静态导入以访问 .Content
...
// 动态导入 Modal 组件以优化初始加载性能
// 这些组件只在用户触发特定操作时才需要加载
const ImagePreviewModal = dynamic(() => import('@/components/common/ImagePreviewModal'), {
  ssr: false,
  loading: () => null,
});

const AssetLibraryModal = dynamic(() => import('@/components/features/library/AssetLibraryModal'), {
  ssr: false,
  loading: () => null,
});
...
export const MainLayout: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const handleAddFilesToLibrary = useCallback(async (files: FileList) => {
      ...
      if (results.some(r => r !== null) && currentView !== 'library') router.push('/library');
  }, [currentView, router, addImagesToLibrary]);
